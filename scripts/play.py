...
def export_policy(cfg: DictConfig, env: "_EnvBase", policy) -> None:
    ...
    three_point_chip = cfg.task.command.get("_target_") == "mimic_lite.ThreePointChipTracking"
    if hasattr(command, "chip") and not three_point_chip:
        policy_config["chip"] = {
    ...
    if cfg.task.command.get("_target_") == "mimic_lite.ThreePointTracking" or three_point_chip:
        policy_config["three_point"] = {
            "format": "mimic_lite_goal_body_three_point_chip_v1" if three_point_chip else "mimic_lite_goal_body_three_point_v1",
            "point_bodies": command.point_names,
        ...
        if three_point_chip:
            policy_config["three_point"]["chip"] = {
                ...
                "training_virtual_target": "nominal_minus_current_world_force_times_compliance",
